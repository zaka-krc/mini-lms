...
<div class="bg-white rounded-lg shadow-lg p-6 max-w-2xl mx-auto">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Créer une Réponse</h1>
        <a href="{{ route('reponses.index') }}" class="text-blue-600 hover:text-blue-800 mt-2 inline-block">
    ...
    <form action="{{ route('reponses.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="text" id="titre" name="titre" value="{{ old('titre') }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('titre') border-red-500 @enderror" placeholder="Texte de la réponse...">
            @error('titre')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            <label class="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" id="is_correct" name="is_correct" value="1" {{ old('is_correct') ? 'checked' : '' }} class="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500">
        ...
        <div>
            ...
            <textarea id="feedback" name="feedback" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('feedback') border-red-500 @enderror" placeholder="Commentaire pour cette réponse...">{{ old('feedback') }}</textarea>
            @error('feedback')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex gap-4 pt-6">
            ...
            <a href="{{ route('reponses.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold py-2 px-6 rounded-lg transition flex items-center gap-2">
