...
@section('title', 'Notes par Apprenant')

@section('content')
<div class="bg-white rounded-lg shadow-lg p-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Notes par Apprenant</h1>
    ...
    <form method="GET" class="mb-6 flex gap-4">
        <select name="apprenant_id" class="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" onchange="this.form.submit()">
            <option value="">-- Tous les apprenants --</option>
            @foreach ($apprenants as $apprenant)
                <option value="{{ $apprenant->id }}" {{ request('apprenant_id') == $apprenant->id || (isset($user) && $user->id == $apprenant->id) ? 'selected' : '' }}>
                    {{ $apprenant->name }}
                </option>
            @endforeach
        </select>
