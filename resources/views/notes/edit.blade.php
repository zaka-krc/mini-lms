...
<div class="bg-white rounded-lg shadow-lg p-6 max-w-2xl mx-auto">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Modifier Note</h1>
    ...
    <form action="{{ route('admin.notes.update', $note) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select id="quiz_id" name="quiz_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('quiz_id') border-red-500 @enderror">
                ...
                @foreach ($quizzes as $quiz)
                    <option value="{{ $quiz->id }}" {{ old('quiz_id', $note->quiz_id) == $quiz->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input type="number" id="note_sur_20" name="note_sur_20" value="{{ old('note_sur_20', $note->note_sur_20) }}" required min="0" max="20" step="0.5"
