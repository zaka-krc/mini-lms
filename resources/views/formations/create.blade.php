...
<div class="bg-white rounded-lg shadow-lg p-6 max-w-2xl mx-auto">
    ...
    <form action="{{ route('admin.formations.store') }}" method="POST" class="space-y-6">
        @csrf

        {{-- Nom --}}
        <div>
            <label for="nom" class="block text-gray-700 font-semibold mb-2">Nom de la formation <span class="text-red-500">*</span></label>
            <input type="text" id="nom" name="nom" value="{{ old('nom') }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('nom') border-red-500 @enderror" placeholder="Ex: Introduction à Laravel">
            @error('nom')
        ...
        <div>
            ...
            <textarea id="description" name="description" required rows="5" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 @enderror" placeholder="Décrivez le contenu de la formation...">{{ old('description') }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="number" id="duree" name="duree" value="{{ old('duree') }}" required min="1" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('duree') border-red-500 @enderror" placeholder="Ex: 20">
            @error('duree')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
