...
<div class="bg-white rounded-lg shadow-lg p-6 max-w-2xl mx-auto">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Créer une Question</h1>
    ...
    <form action="{{ route('admin.questions.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" id="titre" name="titre" value="{{ old('titre') }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('titre') border-red-500 @enderror" placeholder="Posez votre question...">
            @error('titre')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <select id="type" name="type" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('type') border-red-500 @enderror">
                <option value="multiple" {{ old('type') === 'multiple' ? 'selected' : '' }}>QCM (Choix multiples)</option>
        ...
        <div>
            ...
            <textarea id="explication" name="explication" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('explication') border-red-500 @enderror" placeholder="Explication de la bonne réponse...">{{ old('explication') }}</textarea>
            @error('explication')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="number" id="points" name="points" value="{{ old('points', 1) }}" min="1" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('points') border-red-500 @enderror" placeholder="1">
            @error('points')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
