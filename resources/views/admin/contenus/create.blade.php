...
@section('title', 'Nouveau contenu pédagogique')
@section('page-title', 'Nouveau contenu pédagogique')

@section('content')
    <div class="max-w-3xl mx-auto">
        <form action="{{ route('admin.contenus.store') }}" method="POST" class="bg-white rounded-lg shadow p-6">
            @csrf
            ...
            <div class="mb-6">
                ...
                <select id="sous_chapitre_id" name="sous_chapitre_id"
                    ...
                    @foreach($sousChapitres as $sc)
                        <option value="{{ $sc->id }}"
                            {{ old('sous_chapitre_id', $selectedSousChapitreId) == $sc->id ? 'selected' : '' }}>
            ...
            <div class="mb-6">
                ...
                <textarea id="texte" name="texte" rows="10"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-sm @error('texte') border-red-500 @enderror"
                    placeholder="Collez ici le contenu pédagogique (texte, HTML, contenu généré par IA…)">{{ old('texte') }}</textarea>
            ...
            <div class="mb-8">
                ...
                <input type="url" id="lien_ressource" name="lien_ressource" value="{{ old('lien_ressource') }}"
                    placeholder="https://..."
            ...
            <div class="flex gap-4">
                <button type="submit" class="flex-1 bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded-lg transition font-medium">
                    <i class="fas fa-save mr-2"></i> Créer le contenu
                </button>
                <a href="{{ route('admin.contenus.index') }}" class="flex-1 bg-gray-300 hover:bg-gray-400 text-gray-800 px-6 py-3 rounded-lg transition font-medium text-center">
