...
<div class="bg-white rounded-lg shadow-lg p-6 max-w-2xl mx-auto">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Modifier Quiz</h1>
    ...
    <form action="{{ route('admin.quiz.update', $quiz) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" id="titre" name="titre" value="{{ old('titre', $quiz->titre) }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('titre') border-red-500 @enderror" placeholder="Ex: Quiz Chapitre 1">
            @error('titre')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 @enderror" placeholder="Description du quiz...">{{ old('description', $quiz->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="number" id="temps_limite" name="temps_limite" value="{{ old('temps_limite', $quiz->temps_limite ?? 30) }}" min="1" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('temps_limite') border-red-500 @enderror" placeholder="30">
            @error('temps_limite')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="number" id="note_minimale" name="note_minimale" value="{{ old('note_minimale', $quiz->note_minimale ?? 50) }}" min="0" max="100" step="5" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('note_minimale') border-red-500 @enderror" placeholder="50">
            @error('note_minimale')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
